module global_variables
	   ...
	   parameter  (r=1e-9,Q=10,r2=2e-3,r1=1e-3)  !非线性参数r,衰减相关系数Q
	   ...
	   real B,delt,sw,bb
	   real t,x,y,tn,xxx,yyy
	   real Fx,Fy,FA
       real Focus,speed,degree
...
!metal 
     ...
     write(filename,'(I3)') h !以3个字符宽度把h输入到输出位置：filename中
	 open(h,file=adjustr(filename)//".txt",position="rewind") !打开代码为h，文件名为filename.txt的文件，文件打开时将读写移到文件开头(position=rewind的含义)
...
!------------------------ 计算了：vx(m,n),vx(m,n-1),vy(m,n),vy(m-1,n);Tyy(m,n),Txy(m,n),Txx(m,n)(仅在not flag时计算) -------
               else if ((n==1).and.(m>=10).and.(m<=row-10)) 
                   then
			         if ((flag).and.(k<=500000)) then !k肯定小于500000啊？？
                     ...
                     else if ((.NOT.flag).and.(k<=500000)) then !flag：用以判断区域是否为非线性区域？？
                         t=k*deltt/2
                         Focus=sqrt((m-xxx)**2+yyy**2)*deltd            !焦距：xxx\yyy的量纲不是长度，只是表示数量
                         degree=(xxx-m)*deltd/Focus                        !偏转角度正弦值
                         tn=Focus/speed*(1-sqrt(1+((m-xxx)*deltd/Focus)**2-2*(m-xxx)*deltd*degree/Focus))    !时间延迟
                         Fx=A_input*sin(2*pi*f*(t-tn))                                                              !输入信号Fx分量
                         Fy=0
                         Txxf(m,n)=Fx
...
!===================================================================
                     ...
                     else if ((flag).and.(k>500000)) then
                     ...
                     else if ((.NOT.flag).and.(k>50000)) then !k>500000??
                         Tyyf(m,n)=Tyy(m,n)+delt*K1*(vyf(m,n)-vyf(m-1,n))+delt*K2*(vxf(m,n)-vlbxf(m)) !Tyy标准迭代公式
...
!------------------------ 计算了：vx(m,n),vy(m,n);Txx(m,n),Txy(m,n),Tyy(m,n);考虑了衰减：对速度、应力 -------
               else if (m==1.and.n>=100.and.n<=200)then
                   if (flag) then
                       vxf(m,n)=vx(m,n)+delt*(2/(p(m,n)+p(m,n+1)))*(Txx(m,n+1)-Txx(m,n)+Txy(m,n)) !成立条件：Txy(m-1,n)=0，即Tubxy(n)=0
                       vyf(m,n)=vy(m,n)+delt*(2/(p(m,n)+p(m+1,n)))*(Tyy(m+1,n)-Tyy(m,n)+Txy(m,n)-Txy(m,n-1))!vy标准迭代公式
        
                       vxf(m,n)=vxf(m,n)*exp(-pi*fatten*deltt/(2*Q)) !衰减项,为什么其他区域没有考虑衰减？
                       vyf(m,n)=vyf(m,n)*exp(-pi*fatten*deltt/(2*Q))
                   else if (.NOT.flag) then
                       !t=k*deltt/2
					   Txxf(m,n)=Txx(m,n)+delt*K11(m,n)*(vxf(m,n)-vxf(m,n-1))+delt*K2*(vyf(m,n)) !成立条件：vyf(m-1,n)=0，即vuby(n)=0；初始条件：K11=K1
                       Tyyf(m,n)=A_input*sin(2*pi*fL*t) !fL:外加力频率，外加力位于最上端的50~100mm处
                       Txyf(m,n)=Txy(m,n)+delt*U*(vxf(m+1,n)-vxf(m,n)+vyf(m,n+1)-vyf(m,n)) !成立条件：整个区域内的U均相同
					   
                       Txxf(m,n)=Txxf(m,n)*exp(-pi*fatten*deltt/(2*Q))!衰减项
                       Tyyf(m,n)=Tyyf(m,n)*exp(-pi*fatten*deltt/(2*Q))
					   Txyf(m,n)=Txyf(m,n)*exp(-pi*fatten*deltt/(2*Q))
                       
                       K11(m,n)=K1*(1+bb*Txxf(m,n)) !经典非线性对体模量的贡献
                   end if
...
!------------------------ 计算了：vx(m,n),vy(m,n);Txx(m,n),Txy(m,n),Tyy(m,n);考虑了衰减：对速度、应力 -------
               else if (((n>=180.and.n<=200).and.(m>=180.and.m<=200)).or.((n>=280.and.n<=200).and.(m>=180.and.m<=200))) then !判断条件有错！！
                   if (flag) then
                       vxf(m,n)=vx(m,n)+delt*(2.0/(p(m,n)+p(m,n+1)))*(Txx(m,n+1)-Txx(m,n)+Txy(m,n)-Txy(m-1,n))!vx标准迭代公式
                       vyf(m,n)=vy(m,n)+delt*(2.0/(p(m,n)+p(m+1,n)))*(Tyy(m+1,n)-Tyy(m,n)+Txy(m,n)-Txy(m,n-1))
                       
                       vxf(m,n)=vxf(m,n)*exp(-pi*fatten*deltt/(2*Q))   !衰减项
                       vyf(m,n)=vyf(m,n)*exp(-pi*fatten*deltt/(2*Q)) 
                   else if (.NOT.flag) then
                       Tvb=(Txxb(m,n)+Tyyb(m,n))/sqrt(2.0)!Txxb,Tyyb,Txyb似乎没有赋初值？？429~431
                       Tsb=Txyb(m,n)
                       ...
                       Tvf=Tvf*exp(-pi*fatten*deltt/(2*Q)) !衰减项
                       Tdf=Tdf*exp(-pi*fatten*deltt/(2*Q))
					   Tsf=Tsf*exp(-pi*fatten*deltt/(2*Q))
                       ...
                       call  pm_caculate(Tv,Tvf,Hem,O2,Hnum,initial_state,m,n,Kh1,Kh2,Kvf,K11,K2,r1,r2,Tvb,riseflag,1,K_test1) !pm_caculate为一个子程序
                       !pm_caculate()位于文件pm_reaction_rewrite中
...
!------------------------ 以下为迭代(数值传递)过程 -------
        if (.Not.flag) then
            ...
            Tvf=(Txx(180,180)+Tyy(180,180))/sqrt(2.0) !单元(180,180)的体应力
        ...
        if(k==50000)then
          write(h+10,"(1x,500E15.7)") ((vxf(i,j),j=1,column),i=1,row)
          !读写位置右移一位，以科学记数法输出，15个字符宽度，其中小数部分占7位
          !将vxf内存放的所有值，按照原顺序写入文件
          !write(h+20,"(1x,500E15.7)") ((vyf(i,j),j=1,column),i=1,row)
        else if(k==100000)then
            !write(h+10,"(1x,500E15.7)") ((vxf(i,j),j=1,column),i=1,row)
            write(h+20,"(1x,500E15.7)") ((vxf(i,j),j=1,column),i=1,row)
        end if	   
   end do
   ...
   if (1) then !判断条件的含义？？
       do sig_row=2,row-2 !sig_row r=(2,298)
           ...
           write(sig_row+100,"(1x,E15.7)") (v_recsig_x(sig_row,i),i=1,20000) !20000???
           !在代码为r+100的文件中开始写文件
