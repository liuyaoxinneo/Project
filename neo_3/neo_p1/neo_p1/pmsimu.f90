module global_variables
       ...
       integer,parameter::sample=2e5 !采样数
	   ...
	   real Tc(sample),Trr(sample),FF(sample),Kvv(sample),Tm(sample),vr(sample),v_recsig_x(row,sample),v_recsig_y(row,sample)
	   ...
	   parameter  (r=1e-9,Q=10,r2=1e-3,r1=2e-3)  !非线性参数r,衰减相关系数Q
...
program  main_simu
     ...
     integer nd,nu,md,mu
     parameter (nd=180,nu=200,md=180,mu=200)!单个缺陷：n_down<=n<=n_up，m_down<=m<=m_up
     integer remark
...
!------------------------ 计算了：vx(m,n),vy(m,n);Txx(m,n),Txy(m,n),Tyy(m,n);考虑了衰减：对速度、应力 -------
                   ...
                   !其他部分调试已完成，加入非线性迟滞区域 2018.4.11
                   
                   !破损区域:中心在(90,90),10mm*10mm
               else if ((n>=180.and.n<=200).and.(m>=180.and.m<=200)) then 
                   if (flag) then
                       ...
                       Tvf=Tvf*exp(-pi*f*deltt/(2*Q)) !衰减项
...
!------输出文件部分-----     
       do sig_row=2,row-2 !sig_row r=(2,298)
           ...
           write(sig_row+100,"(1x,E15.7)") (v_recsig_x(sig_row,i),i=1,sample)
           !在代码为r+100的文件中开始写文件
