module pm_reaction_rewrite
    ...
    contains !使用contains避免在子程序中重复声明已声明变量
  ...
  if (Tvf>Tv)then       !应力增长时                                       
      Khr_line=0                                                   
      riseflag(m,n)=.true. !凹点无需特殊处理
      if(Tv>A)then         !1、原来的应力是否超出PM空间范围
          K_step=0	
          Khr_line=1/Kh2
      elseif(Tvf<-A)then  !2、增长后的应力是否小于PM空间范围
          K_step=0
          Khr_line=1/Kh1
      elseif(Tvf>A.and.Tv<A.and.Tv>=-A)then  !3、增长后的应力大于最大应力，原应力处于范围中
          Hnum(m,n)=1
          Hem(m,n,1)=A
          O2(m,n,1)=-A
          K_step=-twice_jifen1(Tv,A,-A,density)
          Khr_line=1/Kh2
      elseif(Tv<-A.and.Tvf>-A.and.Tvf<=A)then  !4、原应力小于PM空间最小应力，增长后的应力处于范围中
          Hnum(m,n)=1
          Hem(m,n,1)=Tvf
          O2(m,n,1)=-A
          K_step=-twice_jifen1(-A,Tvf,-A,density)
          Khr_line=-(1/Kh1-1/Kh2)*twice_jifen3(Tvf,A,-A,density)-(1/Kh2)*twice_jifen1(-A,A,-A,density)
      elseif(Tv<-A.and.Tvf>A)then    !5、原应力小于PM空间最小应力，增长后的应力大于最大范围
          Hnum(m,n)=1
          Hem(m,n,1)=A
          O2(m,n,1)=-A
      ...
      elseif(Tv>=-A.and.Tvf<=A)then		 !6、原应力和增长后的应力都处于PM空间范围中                          								  
